...
export default function Index({ eventos }: Props) {
    const proximosEventos = eventos
        .filter((evento) => !esFinalizado(evento))
    ...
    const eventoPrincipal = proximosEventos[0];
    const eventosRestantes = proximosEventos.slice(1);
            ...
            <WebLayout>
                <section className="bg-slate-50 py-16">
                    <div className="mx-auto max-w-6xl px-5 lg:px-8">
                        ...
                        {eventoPrincipal && (
                            <>
                                <div className="mb-6 flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-bold uppercase tracking-widest text-blue-700">Próximo evento</span>
                                    <div className="h-px flex-1 bg-slate-200" />
                                </div>

                                <article className="mb-16 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                                    <div className="grid lg:grid-cols-[1.2fr_1fr]">
                                        <div className="h-72 overflow-hidden bg-slate-200 sm:h-96 lg:h-[440px]">
                                            {eventoPrincipal.imagen_portada ? (
                                                <img
                                                    src={eventoPrincipal.imagen_portada}
                                        ...
                                        <div className="flex flex-col justify-center p-8 lg:p-12">
                                            <div className="mb-5 flex flex-wrap items-center gap-3">
                                                ...
                                                <span className="flex items-center gap-2 text-sm font-medium text-slate-500">
                                                    ...
                                                    {formatearFecha(eventoPrincipal.fecha_inicio)}
                                                </span>
                                            </div>
                                            ...
                                            <div className="mt-7 space-y-3 border-t border-slate-100 pt-6 text-sm text-slate-600">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span>
                                                        {formatearHora(eventoPrincipal.fecha_inicio)}
                                                        {eventoPrincipal.fecha_fin && ` - ${formatearHora(eventoPrincipal.fecha_fin)}`}
                                                    </span>
                                                </div>
                                                ...
                                                {eventoPrincipal.lugar && (
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span>{eventoPrincipal.lugar}</span>
                                                    </div>
                                                )}
                                            </div>

                                            <Link
                                                href={route('web.evento', eventoPrincipal.id)}
                        ...
                        {!eventoPrincipal && eventosFinalizados.length === 0 && (
